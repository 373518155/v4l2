 ...
 *
 * 使用我现在的摄像头，如果使用1280*720的分辨率，对于jpeg格式来说，每秒可达 30帧， 如果用yuyv格式，每秒12帧左右
 *                 如果使用640*480的分辨率，对于yuyv格式，每秒也差不多可达30帧
 */
...
int fd;
int use_format;  // 使用哪一种格式   1 -- jpeg  2 -- yuyv
...
{
    if (argc == 2 && (0 == strcmp(argv[1], "1") || 0 == strcmp(argv[1], "2"))) {
        if (0 == strcmp(argv[1], "1")) {
            use_format = FORMAT_JPEG;
        } else {
            use_format = FORMAT_YUYV;
        }
    } else {
        printf("usage: %s <format>\n\tformat 1 -- jpeg  2 -- yuyv\n", argv[0]);
        return 0;
    }
